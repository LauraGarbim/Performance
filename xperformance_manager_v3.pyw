...
def _flush_disco():
    ...
    try:
        with _save_lock, _MUT_LOCK:
            ...
            try:                                 # .bak atômico (tmp + replace): nunca truncado
                _bak = EXCEL_PATH + ".bak.xlsx"; _bakt = _bak + ".tmp"
                shutil.copy2(EXCEL_PATH, _bakt); os.replace(_bakt, _bak)
            except Exception: pass
    except Exception:
...
def _fechar_db():
    """Fecha a conexão do SQLite na saída limpa (commit final + libera o arquivo)."""
    global _db_conn
    try:
        if _db_conn is not None:
            _db_conn.close(); _db_conn = None
    except Exception:
        pass
atexit.register(_fechar_db)
...
def _clamp_dia(d, mo, ano):
    """Corrige dia impossível para o último dia válido do mês (ex.: 30/02 -> 28/02)."""
    fev = 29 if (ano is None or (ano % 4 == 0 and (ano % 100 != 0 or ano % 400 == 0))) else 28
    maxd = [31, fev, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31][mo - 1]
    return min(d, maxd)
...
def _parse_nasc(nasc):
    ...
    Corrige dia impossível (30/02 -> 28/02) e nunca devolve ano no futuro
    nem idade impossível (nesses casos, ano=None)."""
    s = str(nasc or "").strip()
    ...
    if m:
        ...
        return _clamp_dia(d, mo, ano), mo, ano
...
def _db():
    ...
    with _db_lock:
        if _db_conn is None:
            ...
            # journal_mode=DELETE: o .db fica AUTO-SUFICIENTE (sem -wal/-shm) — backup e
            # sincronização (OneDrive) copiam um único arquivo íntegro. synchronous=FULL:
            # durável a queda de energia. (App é single-writer sob _MUT_LOCK; não precisa WAL.)
...
def _db_save_cadastro(lista, tombstone_ausentes=False):
    ...
    try:
        ...
        existentes = {}; _ilegivel = set()
        for conta, dados in db.execute("SELECT conta,dados FROM cadastro").fetchall():
            dec = _dec_txt(dados)
            if dados and isinstance(dec, str) and dec.startswith(_ENC_TXT):
                _ilegivel.add(conta)   # cifrado mas ILEGÍVEL (ex.: troca de perfil Windows) — preservar
                continue
            try: existentes[conta] = json.loads(dec)
            except Exception: existentes[conta] = {}
        for c in (lista or []):
            k = _conta_str(c.get("conta"))
            if not k or k in _ilegivel: continue   # nunca sobrescreve blob cifrado ilegível
        ...
        if tombstone_ausentes:
            for k in (antigos - atuais) - _ilegivel:
...
def ler_clientes(wb, aba):
    ...
    rows = []
    for row in wb[aba].iter_rows(min_row=3, values_only=True):   # sem max_row: não infla a planilha viva
...
def coletar_por_cliente(wb):
    ...
    Contas com lápide (excluídas) são ignoradas — não reaparecem pelas abas de mês."""
    cad = {_conta_str(c["conta"]): c for c in ler_clientes_cadastro(wb)}
    try:
        _tomb = {r[0] for r in _db().execute("SELECT conta FROM tombstones")}
    except Exception:
        _tomb = set()
    out = {}
...
def _db_reconciliar():
    """Roda a cada abertura. A PLANILHA É A FONTE DA VERDADE:
    1) Cadastro: uma conta apagada da planilha é EXCLUSÃO INTENCIONAL — sai do banco
       e ganha lápide, para NUNCA ressuscitar. (Só propaga se a planilha tiver
       cadastro; planilha vazia/corrompida não apaga ninguém.) O banco deixa de
       restaurar linhas/campos: o que você limpa ou apaga na planilha, fica.
    2) Tabelas (aportes/contatos/receita/controle): ainda restauradas apenas em perda
       TOTAL (aba sumiu/vazia) — rede contra apagão acidental; fora isso, a planilha manda.
    3) Reativar um cliente excluído: basta reprocessar o PDF dele (isso remove a lápide)."""
    try:
        with _MUT_LOCK:
            ...
            # PLANILHA = FONTE DA VERDADE. Conta que está no banco mas foi APAGADA da
            # planilha é exclusão INTENCIONAL: sai do banco e ganha lápide — nunca volta.
            # Guarda de segurança: só propaga exclusões se a planilha tiver cadastro.
            # Planilha vazia/corrompida NÃO apaga ninguém (o banco segue como backup).
            excluidas = []
            if cad:
                banco = {r[0] for r in db.execute("SELECT conta FROM cadastro")}
                for conta in banco - set(por):
                    db.execute("DELETE FROM cadastro WHERE conta=?", (conta,))
                    if conta not in tomb:
                        db.execute("INSERT OR REPLACE INTO tombstones(conta) VALUES(?)", (conta,))
                        excluidas.append(conta)
                if excluidas:
                    db.commit()
                    _db_log(f"_db_reconciliar: {len(excluidas)} conta(s) apagada(s) na planilha "
                            f"propagada(s) ao banco (lápide): {', '.join(excluidas)}")
            # Espelha a planilha no banco (merge não-destrutivo) — a planilha manda.
...
def _idade(nasc):
    """Idade PRECISA (desconta se o aniversário ainda não passou no ano).
    Usa a mesma base de _parse_nasc/_aniv_pack para não divergir entre as telas."""
    p = _parse_nasc(nasc)
    if not p or p[2] is None:
        return None
    d, mo, ano = p
    hoje = datetime.now()
    idade = hoje.year - ano - (1 if (hoje.month, hoje.day) < (mo, d) else 0)
...
# Aceita os dois formatos de título e tolera sufixos de download:
#   "XPerformance - 123 - Ref.30.05.pdf"            (antigo, sem nome)
#   "XPerformance - 123 Fulano - Ref.31.08.pdf"     (novo, com nome do cliente)
#   "... Ref.30.05 (1).pdf" / "... - copia" / "Ref 31 08"  (variações)
_RE_ARQ_CLIENTE = re.compile(
    r"XPerformance\s*-\s*(\d+)(?:\s+([^-]+?))?\s*-\s*Ref\.?\s*(\d{1,2})[.\-/ ]+(\d{1,2})", re.I)

def _parse_arquivo_cliente(fn):
    """(conta, nome, mês) a partir do nome do arquivo; strings vazias se não casar."""
    m = _RE_ARQ_CLIENTE.search(fn or "")
    if not m:
        return "", "", ""
    return m.group(1), (m.group(2) or "").strip(), m.group(4)

def _conta_do_corpo(pdf_path):
    """Blindagem: lê a conta do CABEÇALHO do relatório ('Conta  Assessor  Data de
    Referência' → linha '<conta> <assessor> <data>'), independentemente do nome do
    arquivo. Usa pdfplumber, que extrai o cabeçalho onde o pypdf às vezes falha."""
    try:
        with pdfplumber.open(pdf_path) as pdf:
            t = _page_text_gap(pdf.pages[0]).translate(_LIGADURAS)
    except Exception:
        return ""
    m = re.search(r"Data de Refer[êe]ncia\s+(\d{4,})\b", t)
    if m:
        return m.group(1)
    m = re.search(r"(?m)^\s*(\d{5,})\s+\D", t)
    if m:
        return m.group(1)
    t2 = _ocr_pdf(pdf_path, 1)          # último recurso: OCR (só se Tesseract instalado)
    m = re.search(r"Data de Refer[êe]ncia\s+(\d{4,})\b", t2) or re.search(r"(?m)^\s*(\d{5,})\s+\D", t2)
...
def extrair(pdf_path):
    ...
    A conta é capturada do nome do arquivo e, se falhar, do corpo do PDF (blindagem)."""
    fn = os.path.basename(pdf_path)
    conta, nome_arq, mes = _parse_arquivo_cliente(fn)
    aba = f"{mes.zfill(2)}-{datetime.now().year}" if mes else "Sem-data"
    d = dict(nome=nome_arq, conta=conta, rentabilidade="", ganho="", cdi="", movimentacoes="",
    ...
    try:
        ...
        if not conta:                       # blindagem: conta pelo cabeçalho no texto lido
            mh = re.search(r"Data de Refer[êe]ncia\s+(\d{4,})\b", texto)
            if mh:
                conta = mh.group(1); d["conta"] = conta
    except Exception as e:
        return d, str(e)
    if not conta:                           # último recurso: cabeçalho via pdfplumber
        conta = _conta_do_corpo(pdf_path)
        if conta:
            d["conta"] = conta
    ...
    if aloc:
        d["aloc"] = aloc
    # Blindagem final: não deixar passar em silêncio um relatório sem conta ou sem números.
    if not d["conta"]:
        return d, (f"conta não identificada em '{fn}' — nome do arquivo fora do padrão "
                   f"e cabeçalho do PDF ilegível")
    if not (d["rentabilidade"] or d["patrimonio"] or d["aloc"]):
        return d, (f"'{fn}': não consegui ler os números (possível PDF escaneado/protegido) — "
                   f"conta {d['conta']} ficaria vazia")
    return d, None
...
CLIENT_TO_ALOC = {
    ...
    # Caixa e Proventos são posições líquidas/residuais (benchmark CDI) — contam como
    # Pós-fixado na aderência, em vez de caírem em "fora da carteira" (penalização indevida).
    "caixa":"Pós-fixado", "proventos":"Pós-fixado",
}
...
def _anexo_do_cliente(c, pasta=None):
    """Resolve e VALIDA o relatório do cliente para anexo.
    Retorna (caminho, None) se pode anexar; (None, motivo) se NÃO pode.
    Regra dura: só o relatório da PRÓPRIA conta pode ser anexado (confere pelo
    nome do arquivo e, na dúvida, pelo cabeçalho do PDF)."""
    conta_c = _conta_str(c.get("conta"))
    arq = c.get("arquivo")
    if not arq:
        return None, "sem relatório associado (coluna 'Arquivo PDF' vazia)"
    anexo = _achar_pdf(arq, pasta)
    if not anexo:
        return None, f"PDF não encontrado: {arq}"
    conta_ax = _parse_arquivo_cliente(os.path.basename(anexo))[0] or _conta_do_corpo(anexo)
    if not conta_ax:
        return None, f"não confirmei de quem é o anexo ({os.path.basename(anexo)})"
    if conta_c and conta_ax != conta_c:
        return None, f"o anexo é da conta {conta_ax}, não da conta {conta_c}"
    return anexo, None

def _pdf_por_conta(conta, pasta=None):
    """Procura na pasta um PDF cuja conta (pelo nome OU pelo corpo) seja `conta`.
    Usado no reprocessamento das contas vazias quando o 'arquivo' não bate."""
            ...
            for raiz, _dirs, arquivos in os.walk(base_dir):
                for a in sorted(arquivos):
                    if not a.lower().endswith(".pdf"):
                        continue
                    if (_parse_arquivo_cliente(a)[0] or "") == alvo:
                        return os.path.join(raiz, a)
        except OSError:
            pass
    return None

def _row_to_extrair(c, mes):
    """Converte uma linha lida (ler_clientes) de volta ao formato de extrair(),
    usando os campos numéricos crus — para reescrever a aba sem perder dados."""
    g = lambda k: c.get(k) if c.get(k) is not None else ""
    return {"nome": c.get("nome", ""), "conta": c.get("conta", ""),
            "rentabilidade": g("rent_num"), "ganho": g("ganho_num"), "cdi": g("cdi_num"),
            "movimentacoes": g("mov_num"), "patrimonio": g("pat_num"),
            "rent_total": g("rent_total_num"), "ganho_total": g("ganho_total_num"),
            "periodo_total": c.get("periodo_total", ""), "aloc": c.get("aloc") or {},
            "arquivo": c.get("arquivo", ""), "mes_ref": mes}

def _ocr_pdf(pdf_path, max_paginas=4):
    """OCR de fallback para PDF escaneado. NO-OP (retorna '') se pytesseract/Tesseract
    não estiverem instalados — instale o Tesseract-OCR para ativar."""
    try:
        import pytesseract
    ...
    try:
        ...
        with pdfplumber.open(pdf_path) as pdf:
            for pg in pdf.pages[:max_paginas]:
                out.append(pytesseract.image_to_string(pg.to_image(resolution=200).original, lang="por"))
        return "\n".join(out).translate(_LIGADURAS)
    except Exception:
        return ""

def enviar_via_outlook(dest, assunto, html, anexo=None):
    import win32com.client
    # REGRA DURA: nunca enviar sem o relatório anexado (defesa em profundidade —
    # o chamador já valida, mas o envio em si também se recusa sem anexo válido).
    if not anexo or not os.path.isfile(os.path.abspath(anexo)):
        raise FileNotFoundError("Envio bloqueado: relatório do cliente não anexado.")
...
BACKUP_DIR = os.path.join(_HERE, "_backups")

def _backup_rotativo(manter=30):
    """Snapshot diário do .xlsx + .db numa subpasta, guardando os últimos N dias.
    Roda no boot; só cria o do dia se ainda não existir (idempotente e barato)."""
    try:
        os.makedirs(BACKUP_DIR, exist_ok=True)
        dia = time.strftime("%Y%m%d")
        for src in (EXCEL_PATH, DB_PATH):
            if not os.path.exists(src):
                continue
            dst = os.path.join(BACKUP_DIR, f"{dia}__{os.path.basename(src)}")
            if not os.path.exists(dst):
                tmp = dst + ".tmp"
                shutil.copy2(src, tmp); os.replace(tmp, dst)
        dias = sorted({f.split("__", 1)[0] for f in os.listdir(BACKUP_DIR) if "__" in f})
        for d in dias[:-manter]:                 # rotação: descarta dias além dos últimos N
            for f in os.listdir(BACKUP_DIR):
                if f.startswith(d + "__"):
                    try: os.remove(os.path.join(BACKUP_DIR, f))
                    except OSError: pass
    except Exception:
        _db_log("_backup_rotativo:\n" + traceback.format_exc())

def _autoteste_boot():
    """Check silencioso ao abrir: planilha legível e banco íntegro.
    Devolve '' se ok, ou uma mensagem curta de alerta."""
    problemas = []
    try:
        if os.path.exists(EXCEL_PATH) and not _wb_valido(EXCEL_PATH):
            problemas.append("a planilha parece corrompida (há cópias em _backups)")
    except Exception:
        problemas.append("não consegui validar a planilha")
    try:
        r = _db().execute("PRAGMA integrity_check").fetchone()
        if not r or r[0] != "ok":
            problemas.append("o banco acusou inconsistência")
    except Exception:
        problemas.append("não consegui checar o banco")
...
class Api:
    ...
    def process_pdfs(self, pasta):
        def run():
            try:
                ...
                por_mes = {}
                nao_lidas = []; incompletas = []; ok_n = 0
                for i, fn in enumerate(pdfs, 1):
                    d, e = extrair(os.path.join(pasta, fn))
                    _push("uiProgress", round(i * 100 / len(pdfs)))
                    if not d.get("conta"):
                        nao_lidas.append({"arquivo": fn, "motivo": e or "arquivo fora do padrão"})
                        _push("uiLog", "pdf",
                              f"❌  {fn}: conta não identificada — {e or 'arquivo fora do padrão'}", "err")
                        continue
                    if e:
                        _push("uiLog", "pdf", f"   ⚠  {e}", "warn")
                    completo = bool(d["rentabilidade"] and d["patrimonio"])
                    if completo:
                        ok_n += 1
                    else:
                        incompletas.append({"arquivo": fn, "conta": d["conta"], "mes": d["mes_ref"],
                                            "motivo": "números vazios (PDF ilegível/escaneado?)"})
                    _push("uiLog", "pdf",
                          f"{d['nome'] or '(sem nome)'}  ·  conta {d['conta']}  ·  "
                          f"{d['rentabilidade'] or '—'}%  ·  R$ {d['ganho'] or '—'}  ·  CDI {d['cdi'] or '—'}%",
                          "ok" if completo else "err")
                    por_mes.setdefault(d["mes_ref"], []).append(d)
                # Central de Importação: scorecard estruturado para a tela (clicável).
                _push("uiImport", {"total": len(pdfs), "ok": ok_n,
                                   "incompletas": incompletas, "nao_lidas": nao_lidas,
                                   "pasta": pasta})
                if incompletas or nao_lidas:
                    _push("uiLog", "pdf",
                          f"⚠  {len(nao_lidas)} não lida(s) · {len(incompletas)} incompleta(s) "
                          f"de {len(pdfs)} — veja a Central de Importação.", "warn")
    ...
    def send_emails(self, mes, assunto, corpo, pasta=None):
        def run():
            ...
            try:
                ...
                for c in com:
                    conta_c = _conta_str(c.get("conta"))
                    anexo, motivo = _anexo_do_cliente(c, pasta)
                    if motivo:
                        # REGRA DURA: sem o relatório do PRÓPRIO cliente, o e-mail NÃO sai.
                        _push("uiLog", "mail",
                              f"🛑  NÃO enviado — {c['nome'] or conta_c} (conta {conta_c}): {motivo}.", "err")
                        bloq += 1
                        continue
                    ...
                    try:
                        ...
                        _push("uiLog", "mail", f"✅  {c['email']}  ({c['nome']})  📎", "ok"); ok += 1
                    except Exception as ex:
                        _push("uiLog", "mail", f"❌  {c['email']}: {ex}", "err"); err += 1
                tail = f"{ok} enviado(s)"
                if bloq: tail += f", {bloq} bloqueado(s) sem anexo"
                if err:  tail += f", {err} erro(s)"
                _push("uiLog", "mail", ("✅  " if (err == 0 and bloq == 0) else "⚠  ") + tail + ".",
                      "ok" if (err == 0 and bloq == 0) else "warn")
            except Exception as ex:
    ...
    def mercado_load(self):
        ...
        try:
            ...
            salvar_wb(wb)
            wb2 = _wb_cached()
            aloc = ler_alocacao(wb2); onde = ler_onde_investir(wb2)   # wb2 é o workbook vivo: nunca fechar
    ...
    def dashboard_data(self, incluir_plugados=True):
        ...
        try:
            ...
            pat_list = []; rents = []; rents_tot = []; rents_pond = []
            # Mês de referência único (o mais recente entre todos os clientes): a
            # "rentabilidade média (mês)" considera SÓ os clientes desse mês — não mistura.
            _mreses = set()
            for _o in agg.values():
                _mreses.update((_o.get("meses") or {}).keys())
            mes_ref_global = max(_mreses, key=_ordem_mes) if _mreses else ""
            ...
            for c in cad:
                ...
                if o and o["meses"]:
                    ...
                    if rent is not None and ult == mes_ref_global:
                        rents.append(rent)
                        if pat: rents_pond.append((pat, rent))
            ...
            _somap = sum(p for p, _ in rents_pond)
            rent_media_pond = round(sum(p * r for p, r in rents_pond) / _somap, 2) if _somap else None
            return {"ok": True, "total": len(cad), "aum": aum, "ticket": ticket,
                    "rent_media": rent_media, "rent_media_tot": rent_media_tot,
                    "rent_media_pond": rent_media_pond,
                    "mes_ref": mes_bonito(mes_ref_global) if mes_ref_global else "",
    ...
    def radar(self):
        """Radar do mês: quem precisa de atenção (abaixo do CDI, rentabilidade caiu,
        aporte atrasado, sem contato há 90+ dias, aniversário, aderência crítica)."""
        try:
            if not _tem_dados():
                return {"ok": True, "cats": {}, "total": 0}
            agg = coletar_cache()
            aloc_d = aloc_cache(); tem_aloc = bool(aloc_d.get("classes"))
            lim_ap = _aporte_lim()
            aps = {}
            for a in aportes_cache():
                b = dict(a); b["_d"] = _data_br(b.get("data")); aps.setdefault(str(a.get("conta", "")).strip(), []).append(b)
            cts = {}
            for c_ in contatos_cache():
                b = dict(c_); b["_d"] = _data_br(b.get("data")); cts.setdefault(str(c_.get("conta", "")).strip(), []).append(b)
            cats = {"abaixo_cdi": [], "rent_caiu": [], "aporte_atrasado": [],
                    "sem_contato": [], "aniversario": [], "aderencia_critica": []}
            for conta, o in agg.items():
                nome = o.get("nome") or ("Conta " + conta)
                meses = sorted(o["meses"].keys(), key=_ordem_mes, reverse=True)
                if meses:
                    m0 = o["meses"][meses[0]]
                    cdi = m0.get("cdi_num")
                    if cdi is not None and cdi < 100:
                        cats["abaixo_cdi"].append({"nome": nome, "conta": conta, "info": f"{_brn(cdi)}% do CDI ({mes_bonito(meses[0])})"})
                    if len(meses) >= 2:
                        r0 = m0.get("rent_num"); r1 = o["meses"][meses[1]].get("rent_num")
                        if r0 is not None and r1 is not None and r0 < r1:
                            cats["rent_caiu"].append({"nome": nome, "conta": conta, "info": f"{_brn(r1)}% → {_brn(r0)}%"})
                    if tem_aloc and (m0.get("aloc") or {}):
                        comp = comparar_carteira(m0.get("aloc"), aloc_d, PERFIL_KEY.get(_norm_cls(o.get("perfil", ""))))
                        ad = comp.get("aderencia")
                        if ad is not None and ad < 40:
                            cats["aderencia_critica"].append({"nome": nome, "conta": conta, "info": f"aderência {_brn(ad)}%"})
                st_ap = _aportes_stats(aps.get(conta, []), lim_ap)
                if st_ap.get("alerta"):
                    cats["aporte_atrasado"].append({"nome": nome, "conta": conta,
                        "info": (f"há {st_ap['dias']} dias" if st_ap.get("dias") is not None else "nunca aportou")})
                st_ct = _contato_stats(cts.get(conta, []))
                if st_ct.get("dias") is None or st_ct.get("dias") > 90:
                    cats["sem_contato"].append({"nome": nome, "conta": conta,
                        "info": (f"há {st_ct['dias']} dias" if st_ct.get("dias") is not None else "nenhum contato")})
                p = _aniv_pack((o.get("cad", {}) or {}).get("nascimento"))
                if p.get("aniv_status"):
                    cats["aniversario"].append({"nome": nome, "conta": conta, "info": f"{p['aniv_data']} · {p['aniv_status']}"})
            return {"ok": True, "cats": cats, "total": sum(len(v) for v in cats.values())}
        except Exception as e:
            return {"ok": False, "erro": str(e)}

    def preflight_envio(self, mes, pasta=None):
        """Confere ANTES de enviar: quem tem e-mail e relatório próprio anexável.
        Não envia nada — só devolve o diagnóstico para a tela confirmar."""
        try:
            wb = _wb_cached()
            cl = ler_clientes(wb, mes)
            cad = {str(x["conta"]): x for x in ler_clientes_cadastro(wb)}
            ...
            for c in cl:
                ...
                if not c.get("nome") and info.get("nome"): c["nome"] = info["nome"]
                probs = []
                if not c["email"]: probs.append("sem e-mail")
                _anx, motivo = _anexo_do_cliente(c, pasta)
                if motivo: probs.append(motivo)
                okc = not probs
                if okc: enviaveis += 1
                itens.append({"conta": _conta_str(c["conta"]), "nome": c.get("nome") or "",
                              "email": c.get("email") or "", "ok": okc, "motivo": "; ".join(probs)})
            itens.sort(key=lambda x: (x["ok"], (x["nome"] or "").lower()))
            return {"ok": True, "mes": mes, "total": len(itens),
                    "enviaveis": enviaveis, "bloqueados": len(itens) - enviaveis, "itens": itens}
        except Exception as e:
            return {"ok": False, "erro": str(e)}

    def reprocessar_vazias(self, mes, pasta=None):
        """Relê o PDF só das contas cujos números vieram vazios naquele mês e reescreve
        essas linhas — sem mexer nas contas que já estão OK."""
        def run():
            try:
                refeitas = 0; ainda = []
                with _MUT_LOCK:
                    wb = get_wb()
                    atuais = ler_clientes(wb, mes)
                    lista = []
                    for c in atuais:
                        vazio = c.get("rent_num") is None and c.get("pat_num") is None
                        if vazio:
                            pdf = _achar_pdf(c.get("arquivo"), pasta) or _pdf_por_conta(_conta_str(c.get("conta")), pasta)
                            if pdf:
                                d, e = extrair(pdf)
                                if d.get("conta") and (d["rentabilidade"] or d["patrimonio"]):
                                    if not d.get("nome"): d["nome"] = c.get("nome", "")
                                    d["mes_ref"] = mes
                                    lista.append(d); refeitas += 1; continue
                            ainda.append(_conta_str(c.get("conta")))
                        lista.append(_row_to_extrair(c, mes))
                    if refeitas:
                        escrever_mes(wb, mes, lista); salvar_wb(wb)
                _push("uiLog", "pdf",
                      f"♻  Reprocessadas {refeitas} conta(s) vazia(s)."
                      + (f"  Ainda vazias: {', '.join(ainda)}" if ainda else ""),
                      "ok" if (refeitas and not ainda) else "warn")
                _push("uiRefresh")
            except Exception as ex:
                _push("uiLog", "pdf", f"❌  {ex}", "err")
            finally:
...
<main>
  <header>
    ...
    <button class="btn btn-ghost" onclick="radarShow()" title="Quem precisa de atenção este mês">📡&nbsp; Radar</button>
...
function renderDash(d){
  ...
  const kpiRow=`<div class="dc-row4">
    ...
    ${kbox(KCOL[2], ic.trend,  rmNull?"—":sgn(d.rent_media), rmNull?"#F7F8FA":(rmNeg?LARANJA:CLARO), d.mes_ref?("Rentabilidade média · "+esc(d.mes_ref)+(d.rent_media_pond!=null?" · pond. "+sgn(d.rent_media_pond):"")):"Rentabilidade média (mês)", rmSpark)}
...
function uiImport(d){
  try{
    const host=$("logPdf"); if(!host||!d) return;
    let box=$("importScore");
    if(!box){box=document.createElement("div");box.id="importScore";box.style.margin="8px 0";
      host.parentNode.insertBefore(box,host);}
    const inc=d.incompletas||[], nl=d.nao_lidas||[];
    const probs=nl.map(x=>({a:x.arquivo,m:x.motivo,ic:"❌"})).concat(inc.map(x=>({a:x.arquivo,c:x.conta,m:x.motivo,ic:"⚠"})));
    const meses=Array.from(new Set(inc.map(x=>x.mes).filter(Boolean)));
    let html=`<div style="border:1px solid var(--border);border-radius:10px;padding:10px 12px;background:var(--card)">`
      +`<b>Central de Importação</b> — de ${d.total||0} PDF(s): `
      +`<span style="color:var(--ok)">✅ ${d.ok||0} OK</span> · `
      +`<span style="color:var(--warn)">⚠ ${inc.length} incompleta(s)</span> · `
      +`<span style="color:var(--err)">❌ ${nl.length} não lida(s)</span>`;
    if(probs.length){
      html+=`<div style="margin-top:6px;max-height:150px;overflow:auto;font-size:12px;line-height:1.5">`
        +probs.slice(0,60).map(x=>`<div>${x.ic} ${esc(x.a||"")}${x.c?" · conta "+esc(String(x.c)):""} — ${esc(x.m||"")}</div>`).join("")
        +`</div>`;
    }
    if(inc.length&&meses.length){
      html+=`<button class="btn btn-ghost" style="margin-top:8px" onclick='reprocessarVazias(${JSON.stringify(meses)})'>♻&nbsp; Reprocessar ${inc.length} vazia(s)</button>`;
    }
    box.innerHTML=html+`</div>`;
  }catch(e){}
}
function reprocessarVazias(meses){
  if(_busy)return; uiBusy(true);
  const pasta=$("pasta").value;
  (meses||[]).forEach(m=>api().reprocessar_vazias(m,pasta));
}
async function radarShow(){
  const r=await api().radar();
  if(!r||!r.ok){toast("Erro no radar: "+((r&&r.erro)||""));return;}
  const C=r.cats||{};
  const L={abaixo_cdi:"🔻 Abaixo do CDI",rent_caiu:"📉 Rentabilidade caiu",
    aporte_atrasado:"💸 Aporte atrasado",sem_contato:"📵 Sem contato (90+ dias)",
    aniversario:"🎂 Aniversário",aderencia_critica:"⚠ Aderência crítica"};
  let txt="";
  Object.keys(L).forEach(k=>{
    const arr=C[k]||[];
    if(arr.length){
      txt+=`${L[k]} (${arr.length}):\n`
        +arr.slice(0,15).map(x=>`  • ${x.nome} — ${x.info}`).join("\n")
        +(arr.length>15?`\n  …e mais ${arr.length-15}`:"")+"\n\n";
    }
  });
  modal("Radar do mês — "+(r.total||0)+" ponto(s) de atenção", txt.trim()||"Nada pendente no radar. 🎉", null);
}
...
function processar(){
  if(_busy)return;
  $("logPdf").innerHTML="";const _is=$("importScore");if(_is)_is.remove();
  uiBusy(true);uiProgress(0);
...
async function atualizarPreview(){
  ...
  $("pvMeta").innerHTML=`<b>Para:</b> ${r.email?esc(r.email):"<span style='color:var(--warn)'>sem e-mail cadastrado</span>"}<br><b>Assunto:</b> ${esc(r.assunto||"")}`;
...
async function confirmarEnvio(){
  ...
  // PRÉ-FLIGHT: confere no backend quem tem e-mail E o relatório do próprio cliente anexável.
  const pf=await api().preflight_envio(mes,$("pasta").value);
  if(!pf.ok){toast("Erro na conferência de envio: "+(pf.erro||""));return;}
  if(!pf.enviaveis){toast("Ninguém pronto para envio — todos sem e-mail ou sem o relatório anexável.");return;}
  const bloq=(pf.itens||[]).filter(x=>!x.ok);
  const avisoBloq=bloq.length
    ?`\n\n🛑 ${bloq.length} NÃO serão enviados (o e-mail exige o relatório do próprio cliente):\n`
      +bloq.slice(0,12).map(x=>`• ${x.nome||x.conta} — ${x.motivo}`).join("\n")
      +(bloq.length>12?`\n…e mais ${bloq.length-12}.`:"")
    :"";
  modal("Enviar e-mails",
    `Enviar para ${pf.enviaveis} cliente(s) de ${mes} — cada um com o SEU relatório anexado.`+avisoBloq,
